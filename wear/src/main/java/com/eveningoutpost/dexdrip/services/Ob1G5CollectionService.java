...
public class Ob1G5CollectionService extends G5BaseService {
    ...
    public static void clearKeks() {
        PersistentStore.cleanupOld(KEKS_ONE);
        Plugin.getInstance("9999");
    }

    // ported from com.eveningoutpost.dexdrip.plugin.Dialog.txIdMatch (app-only plugin-consent
    // package not present on wear) - identifies Dexcom ONE+/keks-requiring transmitter ids
    private static boolean txIdMatch(final String txId) {
        ...
        for (BluetoothGattService service : services.getBluetoothGattServices()) {
            ...
            if (service.getUuid().equals(BluetoothServices.CGMService)) {
                ...
                if (txIdMatch(getTransmitterID()) && service.getCharacteristic(ExtraData) != null) {
                    try {
                        plugin = Plugin.getInstance(getTransmitterID());
                        if (plugin == null) {
 ...
 */
    ...
    private static void handleUnknownFirmwareClick() {
        // SendFeedBack/UpdateActivity's nightly-build tracking are phone-only diagnostic
        // conveniences not present on wear - just log the unknown firmware version here.
        ...
        UserError.Log.wtf(TAG, "Unknown firmware version: " + vr1);
    }
